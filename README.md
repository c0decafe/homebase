...
4. The repo now tracks a simple `.envrc` (`use flake`). Run `direnv allow` once and you are ready to go.
...
- **Direnv not loading** - ensure `direnv allow` succeeded (the repo ships an `.envrc` with `use flake`).
